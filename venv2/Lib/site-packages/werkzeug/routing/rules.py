...
class RulePart:
    ...
    either the raw content if *tmp* or a regex string to match
...
class Rule(RuleFactory):
    ...
    `build_only`
        ...
        or folder that are not handled by the WSGI application (like tmp data)
    ...
    def _parse_rule(self, rule: str) -> t.Iterable[RulePart]:
        ...
        while pos < len(rule):
            ...
            if data["tmp"] is not None:
                static_weights.append((len(static_weights), -len(data["tmp"])))
                self._trace.append((False, data["tmp"]))
                content += data["tmp"] if static else re.escape(data["tmp"])
